...
"@
        ...
        function Get-InfoOS {
            ...
            Write-Host "Getting operating system information..." -BackgroundColor Black -ForegroundColor Yellow
        ...
        function Get-Events {
            ...
            Write-Host "Do you want to check event logs?" -BackgroundColor Black -ForegroundColor Red
            $logcheck = Read-Host "[Y]es or [N]o"

            If ($logcheck -eq "Y")
            {
                Write-Host "This will search for 'Error' and 'Warning' entries in SYSTEM and APPLICATION logs." -BackgroundColor Black -ForegroundColor Yellow
                Write-Host "This will search for 'FailureAudit' and 'SuccessAudit' in SECURITY logs." -BackgroundColor Black -ForegroundColor Yellow
                Sleep -Seconds 3
                ...
                $EvtEnd = Read-Host "End Time Reqested (MM/DD/YYYY HH:MM AM/PM)"
                    ...
                    {
                        Write-Host "Checking event logs..." -BackgroundColor Black -ForegroundColor Yellow
                        $evtlogs = Get-EventLog $Log -ComputerName $Computer -After "$EvtStart" -Before "$EvtEnd" -EntryType Error,Warning
                    }
        
                    Else
                    {
                        Write-Host "Checking event logs..." -BackgroundColor Black -ForegroundColor Yellow
                        $evtlogs = Get-EventLog $Log -ComputerName $Computer -After "$EvtStart" -Before "$EvtEnd" -EntryType FailureAudit,SuccessAudit
                    }
            }

            Else
            {
                Write-Host "Skipping event logs..." -BackgroundColor Yellow -ForegroundColor Black
            }

            foreach ($evtlog in $evtlogs){
               $props = @{'Computer Name'=$evtlog.MachineName;
                          'Message'= $evtlog.Message;
                          'Event ID'= $evtlog.EventID;
                          'Entry Type'=$evtlog.EntryType;
                          'Time Written'=$evtlog.TimeWritten;}
        ...
        function Get-InfoCompSystem {
            ...
            Write-Host "Getting system information..." -BackgroundColor Black -ForegroundColor Yellow
            $sys = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $ComputerName
            $proc = Get-WmiObject -Class Win32_Processor -ComputerName $ComputerName
            $props = @{'Model'=$sys.Model;
                       'Manufacturer'=$sys.Manufacturer;
                       'RAM (GB)'="{0:N2}" -f ($sys.TotalPhysicalMemory / 1GB);
                       'Processor'=$proc.Name;
                       'Sockets'=$sys.NumberOfProcessors;
                       'Cores'=$sys.NumberOfLogicalProcessors;
                       'Part of domain'=$sys.PartOfDomain;
                       'Domain name'=$sys.Domain}

            New-Object -TypeName PSObject -Property $props
        }

        # stopped services set to 'AUTO'
        function Get-InfoBadService {
            ...
            Write-Host "Checking services..." -BackgroundColor Black -ForegroundColor Yellow
        ...
        function Get-InstalledPatches {
            ...
            Write-Host "Getting installed patches..." -BackgroundColor Black -ForegroundColor Yellow
            $patches = Get-HotFix -ComputerName $Computer | sort -Descending InstalledOn | Select-Object PSComputerName, HotFixID, InstalledOn, InstalledBy -First 15

            foreach ($patch in $patches){
                $props = @{'Computer Name'=$patch.PSComputername;
                           'HotFix ID'=$patch.HotFixID;
                           'Installed On'=$patch.InstalledOn;
                           'Installed By'=$patch.InstalledBy;}
        ...
        function Get-PerRoutes {
            ...
            Write-Host "Getting persistent routes..." -BackgroundColor Black -ForegroundColor Yellow
            $routes = Get-WmiObject -class Win32_IP4PersistedRouteTable -ComputerName $Computer | Select-Object PScomputerName, Name, Mask, NextHop, Metric1

            foreach ($route in $routes){
                $props = @{'Computer Name'=$route.PSComputername;
                           'Network Address'=$route.Name;
                           'Subnet Mask'=$route.Mask;
                           'Gateway Address'=$route.NextHop;
        ...
        function Get-InfoProc {
            ...
            Write-Host "Getting processes..." -BackgroundColor Black -ForegroundColor Yellow
            $procs = Get-WmiObject -Class Win32_Process -ComputerName $ComputerName | Sort-Object -Descending WS | Select-Object -First 15
        ...
        function Get-InfoNIC {
            ...
            Write-Host "Getting NIC properties..." -BackgroundColor Black -ForegroundColor Yellow
            ...
            forEach($Index in ($NA | Select -Expand Index)){
                $props = @{'Adapter Label'=[String]($na | where {$_.Index -eq $Index}).NetConnectionID
        ...
        function Get-InfoDisk {
            ...
            Write-Host "Getting drive information..." -BackgroundColor Black -ForegroundColor Yellow
        ...
        function Get-Connections {
            ...
            Write-Host "Checking connection to gateway..." -BackgroundColor Black -ForegroundColor Yellow
            $gw = Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $ComputerName | Where {$_.DefaultIPGateway -notlike "$null"} | Select-Object -Expand DefaultIPGateway
            $sip = Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $ComputerName | Where {$_.DefaultIPGateway -notlike "$null"} | Select-Object -Expand IPaddress
            $connections = Test-Connection $gw -Count 2 | Select-Object PSComputerName,ResponseTime

            foreach ($connection in $connections) {
                $props = @{'Source IP Address'=$sip;
        ...
        foreach ($computer in $ComputerName) {
            ...
            If ($everything_ok) {
                ...
                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Event Logs</h2>';
                            ...
                            'Message',
                            'Time Written'}
                $html_el = Get-Events -ComputerName $Computer | ConvertTo-EnhancedHTMLFragment @params

                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Connection to Gateway</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid';
                            'Properties'='Source IP Address',
                            'Gateway IP Address',
                            'Response Time (ms)'}
                $html_ob = Get-Connections -ComputerName $Computer | ConvertTo-EnhancedHTMLFragment @params

                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Disk Space Report</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid';
                ...
                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Top 15 Processes by Memory Used</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid'}
                $html_ip = Get-InfoProc -ComputerName $computer | ConvertTo-EnhancedHTMLFragment @params
                    
                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Persistent Routes</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid';}
                $html_pr = Get-PerRoutes -ComputerName $Computer | ConvertTo-EnhancedHTMLFragment @params
                
                $params = @{'As'= 'Table';
                            'PreContent'='<h2>&bull; Services in Stopped State That are Set to "Auto"</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid'}
                ...
                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; NIC Information</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid'
                ...
                $params = @{'As'='Table';
                            'PreContent'='<h2>&bull; Recently Installed Patches</h2>';
                            'EvenRowCssClass'='even';
                            'OddRowCssClass'='odd';
                            'MakeTableDynamic'=$true;
                            'TableCssClass'='grid'}
                $html_pa = Get-InstalledPatches -ComputerName $Computer | ConvertTo-EnhancedHTMLFragment @params
